...
class BatchNorm1d:
    ...
    def forward(self, Z, eval=False):
        ...
        self.M = (1/self.N) * np.sum(Z, axis = 0)  # TODO: Calculate mini-batch mean
        self.V = (1/self.N) * np.sum(np.square(Z - self.M ), axis = 0)  # TODO: Calculate mini-batch variance
    ...
    def backward(self, dLdBZ):
        ...
        """
        self.dLdBb = np.sum(dLdBZ, axis = 0)  # TODO: Sum over the batch dimension.
        self.dLdBW = np.sum(dLdBZ * self.NZ, axis = 0)  # TODO: Scale gradient of loss wrt BatchNorm transformation by normalized input NZ.

        dLdNZ = dLdBZ * self.BW # TODO: Scale gradient of loss wrt BatchNorm transformation output by gamma (scaling parameter).

        #dLdV =  - 0.5 * np.sum(self.dLdBW * (self.Z - self.M) * np.power((self.V + self.eps), -3/2) ,axis = 1)   # TODO: Compute gradient of loss backprop through variance calculation.
        dLdV = np.sum(dLdNZ * (self.Z - self.M) * (-0.5) * (self.V + self.eps) ** (-1.5), axis=0)
        
        dNZdM = - np.power((self.V + self.eps), -1/2) - 1/2 * (self.Z - self.M) * np.power((self.V + self.eps), -3/2) * (-2/self.N) * np.sum((self.Z - self.M), axis = 0) # TODO: Compute derivative of normalized input with respect to mean.
        dLdM = np.sum(dLdNZ * dNZdM , axis = 0)  # TODO: Compute gradient of loss with respect to mean.

        #dLdZ = dLdNZ * (np.power((self.V + self.eps), -1/2)) + dLdV * ((2/self.N) * (self.Z - self.M)) + (1/N)*dLdM # TODO: Compute gradient of loss with respect to the input.
        dLdZ = (dLdNZ * (self.V + self.eps) ** (-0.5) + dLdV * (2.0 / self.N) * (self.Z - self.M) + (1.0 / self.N) * dLdM[np.newaxis, :])
